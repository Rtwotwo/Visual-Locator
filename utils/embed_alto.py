"""
任务: 处理alto数据集,将queries和reinference文件夹数据
...
def parsers():
    ...
    parser.add_argument('--dataset_name', type=str, default='tianzhibei', 
                        help="there are two datasets you can choose ['tianzhibei', 'UAV', 'nwpu']")
...
class AltoDataset(Dataset):
    ...
    def __init__(self, args, transform=None, **kwargs,):
        ...
        if args.model_name == 'selavpr':
            ...
            # Prithvi_100M model input image should satisfied with 6 channels
    ...
    def __getalto__(self):
        # specified the code for alto dataset
        self.embeddings, self.address = [],[]
        self.alto_embed_paths = None
        if self.args.dataset_proc == 'test':
            self.dataset_paths = os.listdir(self.dataset_model_path)
            # the dataset_model_path(parent dir) has no children dirs
            for path in tqdm(self.dataset_paths, desc='====processing test data'):
                full_path = os.path.join(self.dataset_model_path, path)
            ...
            print(f'====test dataset embeddings shape: {self.embeddings.shape}')
            self.alto_embed_paths = {'embeddings':self.embeddings,'address':self.address}
                
        elif self.args.dataset_proc == 'train':
            ...
            for dir in database_query_dirs:
                if dir == 'database' and dir == self.args.dataset_mode:
                    full_dir = os.path.join(self.dataset_model_path,
                                'database', self.args.dist_offset)
    ...
    def __getnwpu__(self):
        ...
        elif self.args.dataset_proc == 'test':
            # the dataset_model_path(parent dir) with children dirs
            database_query_dirs = os.listdir(self.dataset_model_path)
            for dir in database_query_dirs:
                if dir == 'database'and dir == self.args.dataset_mode:
    ...
    def __save_altodata__(self):
        # save the processed dataset data: embeddings and image_paths
        if args.dataset_name == 'alto':
            embed_paths = self.__getalto__()
        elif args.dataset_name == 'nwpu':
            embed_paths = self.__getnwpu__()
        else: raise RuntimeError(f'No founded the {args.dataset_name}, please reset it.')
        ...
        self.use_h5py_save_features(embed_paths, self.save_filepath)
        print('====All done!')
...
########################### 主程序测试 #######################################
# python embed_alto.py --dataset_name=UAV --dataset_proc=train              #
...
if __name__ == '__main__':
    ...
    alto_embed_paths = alto_dataset.__save_altodata__()