...
class GeoLocalizationNet(nn.Module):
    ...
    def __init__(self, weight_path=None):
        ...
        if weight_path is not None:
            self.backbone = get_backbone(weight_path=weight_path)
        else:raise ImportError("There is correct dinvo2 pretrained model weight path")
...
def get_backbone(weight_path=None):
    # if args.registers:
    ...
    backbone = vit_large(patch_size=14,img_size=518,init_values=1,block_chunks=0) 
    foundation_model_path = weight_path
    if foundation_model_path is not None:
        model_dict = backbone.state_dict()
        
        # use the tripnet loss trained model state_dict
